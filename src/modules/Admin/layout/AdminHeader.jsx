...
export default function AdminHeader({ onToggleSidebar }) {
  ...
  const doLogout = () => { logoutAdmin(); nav("/admin/login", { replace: true }); };

  return (
    <header className="h-14 flex items-center justify-between px-4 border-b border-charcoal/20 bg-white">
      <div className="flex items-center gap-3">
        <button onClick={onToggleSidebar} className="p-1.5 rounded hover:bg-brand/10" aria-label="Toggle sidebar">
      ...
      <div className="flex items-center gap-2">
        <button className="p-1.5 rounded hover:bg-brand/10" aria-label="Notifications">
          <Bell className="h-5 w-5 text-ink" />
        </button>
        <button onClick={doLogout} className="inline-flex items-center gap-1 rounded bg-brand px-3 py-1.5 text-sm font-medium text-ink hover:opacity-90">
