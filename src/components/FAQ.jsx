...
function Item({ i, q, a }) {
  ...
  return (
    <div className="border-b border-charcoal/15">
      ...
      {open && (
        <div id={id} role="region" className="pb-4 text-sm text-slate">
          {a}
        </div>
      )}
    </div>
  ...
  return (
    <section id="faq" className="bg-white">
      <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-14">
        <h2 className="text-3xl font-bold text-center text-ink">Frequently Asked Questions</h2>
        <div className="mt-8 rounded-2xl border border-charcoal/15 bg-white shadow-sm">
          {faqs.map((f, i) => <Item key={i} i={i} {...f} />)}
        </div>
      </div>
    </section>
  );
